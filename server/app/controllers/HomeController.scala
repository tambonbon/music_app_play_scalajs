...
class HomeController @Inject()(cc: ControllerComponents,
  ...
  ************* */
  ...
  //TODO Implement FullAlbum instead of doing this error-prone method of adding album/song separately
...
//          songDAO.addSong(data.title, data.duration)
          albumSongDAO.normalized(Await.result(albumDAO.getIdMostRecentAlbum, 0.1 seconds),
            Await.result(songDAO.getIdMostRecentSong ,0.1 seconds)) // THIS IS NOT RECOMMENDED
  ...
  def addAlbumJson = Action.async(parse.json) { request =>
    ...
    response.fold(
      ...
      newAlbum => { //TODO this should be NO id
        val albumFut = albumDAO.addAlbum(newAlbum.artist, newAlbum.name, newAlbum.genre)
        for (
          song <- newAlbum.songs
        ) yield {
          val songFut = songDAO.addSong(song.name, song.duration)
          songFut
//          val songIdFut = songFut.map(_.id)
          for {
            album <- albumFut
            song <- songFut
          } yield albumSongDAO.normalized(album.id, song.id)
//          albumSongDAO.normalized(Await.result(albumFut.map(_.id), 0.5 seconds), Await.result(songIdFut, 0.5 seconds))
        }
